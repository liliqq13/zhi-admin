...
const CACHE = PREFIX + 'f5555a472eb03627-reload';
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(ASSETS.map(path => new Request(new URL(path, base).href, {cache:'reload'})))));
});
